  ...
  try {
    ...
    const parseParticipants = (input: any): string[] => {
      ...
      if (typeof input === 'string') {
        ...
        return cleaned.split(',').map(e => e.replace(/(^"|"$)/g, '').trim()).filter(Boolean);
      }
      return [];
    };
    ...
    const start = new Date(start_time).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
    const end = new Date(end_time).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
